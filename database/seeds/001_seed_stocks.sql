...
-- 35개 핵심 기업 기본 정보 + 수집 대상(is_target)/약칭(short_name) 설정.
-- 재실행 안전(idempotent): ON CONFLICT 기반.
-- universe 확장(15→27): jasoseol 커버리지 ≥15 기준 Tier-1 12종목 추가(2026-06-25, A 결정).
-- universe 확장(27→35): Neon 수집 DB 35종과 정합 위해 8종 추가(2026-07-02).

-- 1. 35개 핵심 기업 기본 정보 주입 (이미 테이블에 있으면 sector만 최신화)
INSERT INTO stocks (ticker, name, market, sector) VALUES
    ...
    -- universe 확장 8종 (Neon 수집 DB 35종 정합, 2026-07-02)
...
SET is_target = TRUE,
    short_name = CASE ticker
        ...
        -- universe 확장 8종: '삼성'·'한화'·'현대' 단독 약칭은 기존 종목과 충돌하므로 금지.
        -- 특정적 약칭만 부여(HD·非HD 표기 커버), 나머지는 name 으로 충분해 NULL 유지.
        WHEN '012450' THEN '한화에어로'     -- '한화'(한화오션 충돌) 금지
        WHEN '055550' THEN '신한'           -- 지주사: 공고가 신한은행/카드/증권 명의 → 그룹 커버
        WHEN '105560' THEN 'KB'             -- 지주사: 공고가 KB국민은행/증권 명의 → 그룹 커버
        WHEN '267260' THEN '현대일렉트릭'   -- '현대'(현대차/모비스 충돌) 금지
        WHEN '329180' THEN '현대중공업'     -- '현대' 금지
        -- 009150 삼성전기·034020 두산에너빌리티·042660 한화오션 → NULL(name 특정적)
...
WHERE ticker IN (
    ...
    '003670','032830',
    -- universe 확장 8종 (Neon 35종 정합, 2026-07-02)
    '009150','012450','034020','042660','055550',
    '105560','267260','329180'
);
