...
CREATE TABLE public.meta_signals (
    ...
    CONSTRAINT meta_signals_direction_check CHECK (((direction IS NULL) OR ((direction)::text = ANY ((ARRAY['positive'::character varying, 'negative'::character varying, 'neutral'::character varying, 'unknown'::character varying])::text[])))),
    CONSTRAINT meta_signals_method_check CHECK (((method)::text = ANY ((ARRAY['stacking'::character varying, 'equal_fallback'::character varying, 'empty'::character varying, 'linear_stacking'::character varying])::text[])))
);
...
CREATE TABLE public.recommendations (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT recommendations_basis_check CHECK (((basis)::text = ANY ((ARRAY['final'::character varying, 'meta'::character varying])::text[]))),
    ...
    CONSTRAINT recommendations_signal_check CHECK (((signal)::text = ANY ((ARRAY['positive'::character varying, 'negative'::character varying, 'neutral'::character varying, 'mixed'::character varying])::text[])))
);
