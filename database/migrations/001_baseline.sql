...
-- ----------------------------------------------------------------------------
-- 기존 001~016 마이그레이션을 하나의 베이스라인으로 squash 한 파일이다.
-- 개발 단계라 운영 데이터가 없으므로 증분 마이그레이션 이력을 단일화한다.
--
-- 통합 규칙(자세한 내용은 database/docs/migration_rules.md):
--   - 014(hiring_raw_details.observed_date ALTER)는 003 CREATE TABLE에 흡수했다.
--   - 015/016의 `IF NOT EXISTS`는 컨벤션(§3)대로 제거했다 (fresh DB 기준).
...
CREATE TABLE hiring_signals (
    ...
    is_spike          BOOLEAN NOT NULL DEFAULT FALSE,
    created_at        TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_hiring_signals_stock_date UNIQUE (stock_id, observed_date)
);
...
CREATE TABLE hiring_sources (
    ...
    is_active     BOOLEAN NOT NULL DEFAULT TRUE,
    created_at    TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_hiring_sources_stock_crawler UNIQUE (stock_id, crawler_type)
);
