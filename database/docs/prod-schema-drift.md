...
> ⚠️ **스테일 주의 (#531 재베이스라인 이후):** 본 문서는 구 **단일 `001_baseline.sql`(스쿼시)**
> 기준으로 작성됐다. 이후 2-인스턴스 분리(#531)로 베이스라인이 `0001`~`0007`(수집/백엔드 타깃별)로
> 재구성됐고, 운영도 Supabase 단일 → Cloud SQL ×2 로 이동 중이다. 따라서 아래 "어떤 마이그가
> prod 에 있나/없나" 분석은 **현재 베이스라인 구조에 그대로 대응하지 않는다**. Cloud SQL 그린필드
> 부트스트랩(`runbooks/db-2-instance-bootstrap.md`)으로 새로 적재하는 경우 이 드리프트 항목 대부분은
> 무의미하다. 기존 prod 를 이관한다면 **현재 베이스라인 기준으로 드리프트를 재확인**할 것.
>
> 상태: **팀 조치 필요(재확인 필요)**. 작성자: C2 polarity 적용 작업 중 발견.
