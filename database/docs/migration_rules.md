...
개발 단계라 운영 데이터가 없으므로, 그동안 누적된 `001`~`016` 16개 마이그레이션을
**단일 `001_baseline.sql` 한 파일로 squash** 했다.

| 항목 | 처리 |
| --- | --- |
| 014 ALTER (`hiring_raw_details.observed_date`) | 003의 `CREATE TABLE`에 컬럼·인덱스로 **흡수** (ALTER 제거) |
| 015 `hiring_signals` / 016 `hiring_sources` | 베이스라인에 합치고 `IF NOT EXISTS`는 컨벤션(§3)대로 제거 |
...
결과: 마이그레이션 파일 **1개**(`001_baseline.sql`) + 시드 5개. 총 **50개 테이블**.
