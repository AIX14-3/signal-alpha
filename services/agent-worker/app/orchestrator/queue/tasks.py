...
DEFAULT_CYCLE_PLAN: dict[str, int] = {
    ...
    # PRICE — 빠름(DB만 읽음), 큰 캡
    "analyze_price": 50,
    # LLM 코호트 채점 — 태스크 1개 = 코호트 N종목(LLM 1콜)이라 캡 20이면 하루치 유니버스를
    # 넉넉히 덮는다. 이 키가 없으면 라이브 데몬(plan 기반)이 SCORE_COHORT 를 영영 claim 못
    # 한다(requery_source 선례). 플래그 off 면 태스크가 시드되지 않아 no-op.
    "score_cohort": 20,
