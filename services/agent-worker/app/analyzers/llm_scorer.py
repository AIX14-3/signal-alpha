...
SCHEMA_RETRIES = 1
"""스키마 위반 재시도 횟수(계약 위반 사유를 프롬프트에 되먹여 재질의).

실측(gemini-2.5-flash, 42콜): **10회(24%)가 'scores' 키 누락**으로 실패했다. 전송 오류가 아니라
모델이 계약을 안 지킨 것이라 클라이언트의 HTTP 재시도로는 안 잡힌다.

⚠️ **이 재시도가 스키마 위반을 실제로 줄이는지는 아직 검증되지 않았다.** 검증 런이 API 크레딧
고갈(HTTP 429)로 오염돼 결과를 읽을 수 없었다. 오히려 재시도가 호출량을 배로 늘려 레이트리밋을
앞당기는 역효과가 관측됐다 — 그래서 2회에서 **1회로 낮췄다**. 재검증 전까지 이 값을 올리지 말 것.

스키마 준수율 자체는 **모델 선정의 1순위 지표**다(같은 프롬프트에서 계약을 못 지키는 모델은
운영에 쓸 수 없다). 모델 비교 매트릭스에서 이 값을 측정한다."""
