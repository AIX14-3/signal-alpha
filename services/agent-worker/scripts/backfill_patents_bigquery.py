...
def _bq_rows(
    *, start_year: int, end_year: int, patterns: list[str], project: str,
    ...
    ``date_field`` selects which date the [start_year, end_year] window filters on.
    Default ``publication_date`` (when the patent text became PUBLIC) is the
    point-in-time-correct choice for keyword mining — a 2016 keyword must come from
    text public by 2016, not merely filed by then (filing lags publication ~18mo)."""
    if date_field not in ("publication_date", "filing_date"):
        raise SystemExit(f"date_field must be publication_date or filing_date, got {date_field!r}")
    try:
...
async def _run(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--date-field", choices=["publication_date", "filing_date"], default="publication_date",
        help="window filter date; publication_date (default) = point-in-time-correct for keyword mining",
    )
    ...
    print(f"[bq] querying {BQ_TABLE} (project={project}) {args.date_field} {args.start_year}-{args.end_year}, tickers={tickers}")
    rows = _bq_rows(
        start_year=args.start_year, end_year=args.end_year, patterns=all_patterns,
        project=project, date_field=args.date_field,
    )
        ...
        Path(args.out).write_text(
            json.dumps(
                {t: [{"application_no": r.application_no, "title": r.invention_title,
                      "application_date": r.application_date, "publication_date": r.open_date,
                      "ipc": r.ipc_code, "applicant": r.applicant_name} for r in recs]
