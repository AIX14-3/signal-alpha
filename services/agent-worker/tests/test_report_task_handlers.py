...
class ReportProcessTaskHandlerTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_does_not_enqueue_embed_report_when_llm_enabled(self):
        ...
        # Gate OFF (settings has no report_use_llm) → no embed_report enqueue.
        conn_off = ProcessHandlerConn()
        handler_off = ReportProcessTaskHandler(
            connection=conn_off, settings=object(), storage=FakeStorage(exists=True)
        )
        result_off = await handler_off({"task_context": {"raw_document_id": 42}})
        types_off = [args[1] for _, args in conn_off.fetchvals]
        self.assertIn("normalize_report", types_off)
