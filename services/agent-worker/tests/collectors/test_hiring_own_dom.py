"""자체 DOM 공식 채용관 파서 단위 테스트 (#175 PoC: 현대).

- 현대 React 채용관(ul.apply__list)의 data-* 속성 기반 파싱
- 일반/그룹 공고 상세 URL 분기
- closing_date(YYYY-MM-DD) 정규화
- CollectorResult 로 raw_payload·source_label 보존(4b reparse 호환)
- 공고 없음(board-empty) 시 빈 리스트 반환
"""
...
from app.collectors.hiring.sites.company.hyundai_kia import HyundaiCrawler

_FIXTURE = Path(__file__).resolve().parents[1] / "fixtures" / "hiring" / "HYUNDAI.html"
