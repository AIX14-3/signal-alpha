...
class DartCollectionSchedulerTest(unittest.IsolatedAsyncioTestCase):
    async def test_enqueue_due_collections_for_active_stocks(self):
        ...
        self.assertEqual(
            queue_repository.calls[0]["task_context"],
            {"stock_code": "005930", "end_de": "20260610"},
        )
    ...
    async def test_enqueue_due_collections_can_include_structured_dart_collections(self):
        stock_repository = FakeStockRepository()
        queue_repository = FakeQueueRepository()
        scheduler = DartCollectionScheduler(
            stock_repository=stock_repository,
        ...
        self.assertEqual(result["scheduled_count"], 8)
        self.assertEqual(result["task_ids"], [101, 102, 103, 104, 105, 106, 107, 108])
        self.assertEqual(result["collect_dart_task_ids"], [101, 105])
        self.assertEqual(result["collect_dart_ownership_task_ids"], [102, 106])
        self.assertEqual(result["collect_dart_financials_task_ids"], [103, 107])
        self.assertEqual(result["collect_dart_employee_task_ids"], [104, 108])
        self.assertEqual(
            [call["task_type"] for call in queue_repository.calls[:4]],
            [
                "collect_dart",
        ...
        self.assertEqual(queue_repository.calls[2]["stock_id"], 1)
        self.assertEqual(queue_repository.calls[2]["task_context"], {"stock_code": "005930"})
        self.assertTrue(queue_repository.calls[2]["dedupe"])
        self.assertEqual(queue_repository.calls[3]["task_context"], {"stock_code": "005930"})
