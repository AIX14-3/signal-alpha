...
def test_collect_dart_ownership_is_drained_between_dart_collect_and_normalize():
    ...
    assert NORMALIZE_DART_FINANCIALS == "normalize_dart_financials"
    assert NORMALIZE_DART_EMPLOYEE == "normalize_dart_employee"
    ...
    assert DEFAULT_CYCLE_PLAN[NORMALIZE_DART_FINANCIALS] == 10
    assert DEFAULT_CYCLE_PLAN[NORMALIZE_DART_EMPLOYEE] == 10
    ...
    assert DRAIN_ORDER.index(NORMALIZE_DART_OWNERSHIP) < DRAIN_ORDER.index(
        NORMALIZE_DART_FINANCIALS
    )
    assert DRAIN_ORDER.index(NORMALIZE_DART_FINANCIALS) < DRAIN_ORDER.index(
        NORMALIZE_DART_EMPLOYEE
    )
    assert DRAIN_ORDER.index(NORMALIZE_DART_EMPLOYEE) < DRAIN_ORDER.index(NORMALIZE_DART)
