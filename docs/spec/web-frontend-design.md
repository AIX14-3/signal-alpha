...
- **잠금 오버레이**(신규): 카드 위 `backdrop-filter: blur(6px)` + 반투명 surface + 자물쇠 아이콘 + CTA 버튼. 비회원/미언락 소스에 적용.
- **쿼터 배지**(신규): pill, `surface-2` 배경, "무료 N회 남음"(잔여 0이면 red 강조) / 구독 시 sky pill "구독 중".
...
2. **리포트(`/report/[ticker]`)**: 상단 종목 헤더 + 종합 게이지(좌) / 요약(우). 하단 **5소스 카드 그리드**(주식정보·DART·채용공고·네이버 키워드·증권사 리포트) 각 방향 칩 + 점수 + 1–2줄 LLM 요약 + "상세 보기". 회원 미언락/비회원은 잠금 카드. 우상단 쿼터 배지 + "발행" 버튼.
3. **소스 상세(`/report/[ticker]/[source]`)**: 소스 헤더(라벨/방향/점수) + LLM 상세 요약 카드 + 원천 데이터(테이블 또는 Recharts). 비회원 잠금(dart/datalab 제외).
...
6. **가격/결제(`/pricing`)**: 단일 9,900원 카드(무제한 열람) + 무료(3회) 비교. 결제 버튼 → 포트원 결제창.
7. **관리자(`/admin`)**: 로그인 게이트 → 매출 지표(Recharts 카드) + 회원 테이블(검색/페이지네이션) + 구독 관리 액션.
8. **비회원 블라인드 리포트**: 2번 레이아웃에서 DART·네이버 카드만 선명, 나머지 3 + 종합 게이지에 잠금 오버레이 + "로그인하고 무료 3회 열람" CTA.
...
| 패턴 | 사용처 | 표현 |
|---|---|---|
| 잠금 오버레이 | 비회원/미언락 소스·종합점수 | 블러 + 자물쇠 + CTA |
| 쿼터 배지 | 리포트 상단·마이페이지 | pill, 잔여/구독 상태 |
| 쿼터 소진 모달 | 402 응답 | 구독 유도 |
...
- 이번 추가: `signal_alpha_minimal_2026_v21_report_sources.html` — 신규 리포트(5소스 + 쿼터/블라인드/소스 상세/본인인증/마이페이지) 패턴을 한 파일에 시연.
