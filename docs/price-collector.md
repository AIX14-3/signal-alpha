...
- **중복 기동 방지**: Postgres advisory lock을 잡은 인스턴스만 폴링합니다.
  uvicorn 멀티 워커·컨테이너 중복이 있어도 한 인스턴스만 수집하고, 보유자가
  죽으면 락이 풀려 60초 안에 다른 인스턴스가 승계합니다.
- `DATABASE_URL` 또는 `KIWOOM_APP_KEY/SECRET`이 비어 있으면 데몬은 경고 로그
  한 줄을 남기고 기동하지 않습니다 (장중 실패 스팸 방지).
