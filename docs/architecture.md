...
signal-alpha/
  ...
  database/
    migrations/           # PostgreSQL + pgvector 스키마 (유일한 기준)
...
```text
[Next.js web]  ──HTTP──▶  [main-server]  ──내부 API──▶  [agent-worker]  ──▶  PostgreSQL+pgvector
                              (사용자 경계)               (수집/분석 경계)
```
...
- **`agent-worker`** 는 수집·정규화·분석·큐·LLM/RAG·키움 가격 수집 데몬을 담당합니다. 현재 라우트 그룹:
...
사용자 입력: stock_name / stock_code
  ...
  ┌──────────────── Fan-out 병렬 분석 ────────────────┐
  │  DART Watcher   │  Report RAG   │  Alternative Signal │
  └──────────────────────────────────────────────────┘
        ↓
...
| 분석 | Rule-based analyzer + 선택적 LLM 분석 + RAG |
| LLM Provider | 교체 가능한 Provider 추상화 (Gemini 등) |
| Embedding / Vector | BGE-M3(1024 dim) + PostgreSQL `pgvector` |
| DB 접근 | `packages/data-access` repository |
| 의존성 관리 | **uv 워크스페이스** (`pyproject.toml` + `uv.lock` 단일 출처) |
| 로컬 인프라 | Docker Compose (postgres+pgvector, 서비스들) |
...
4. LLM은 고임팩트 공시·RAG 요약처럼 필요한 지점에만 사용하고, JSON 검증 실패/타임아웃/금지 표현 감지 시 결정적 fallback을 씁니다.
