...
- MVP는 삼성전자, SK하이닉스, 네이버 3개 종목과 mock/fallback 데이터로 E2E 데모를 먼저 완성한다.
- 외부 데이터 수급은 1순위로 검증하되, 막히면 즉시 fixture/mock 기반 데모 경로를 유지한다.
...
- [ ] source별 mock `RawEvidence` fixture 생성
- [ ] source별 mock `SourceResult` fixture 생성
- [ ] collect 단계 fan-out 구현
- [ ] analyze 단계 fan-out 구현
- [ ] aggregation 단계 구현
- [ ] partial 실패 처리
...
- [ ] API 실패용 DART raw fixture 작성

**Analyzer 작업**

- [ ] 고임팩트 공시 유형 분류
- [ ] 실적/주요사항 분석 fixture 작성
- [ ] 공식 데이터 기준 방향성 산출
- [ ] DART source result 생성

**완료 기준**

- [ ] 삼성전자, SK하이닉스, 네이버 중 최소 1개 종목의 실제 DART 호출이 성공한다.
- [ ] DART Collector 실패 시 fixture 기반 `raw_evidence`가 저장된다.
...
- [ ] 최소 1개 Alternative source가 실제 호출 또는 fixture로 동작한다.
...
**목표**: 팀원 누구나 어려운 법률·개발 용어 없이 PR 규칙을 이해하고, 항상 같은 형식으로 PR을 올리게 만든다.

- [ ] `.github/PULL_REQUEST_TEMPLATE.md` 추가 (gitmore 표준 구조 + Signal α 안전 체크리스트)
- [ ] `docs/pr-guide.md` 추가 — 어려운 PR 규칙을 "쉬운 말"로 번역한 가이드
- [ ] "어려운 용어 → 쉬운 말 사전" 섹션 작성 (예: 투자자문업 / 상수화 / 금지 표현 필터)
- [ ] PR 제목·브랜치 네이밍 규칙 정의
- [ ] PR 템플릿이 자동으로 떠서 규칙을 "항상 읽게" 되는지 확인

**완료 기준**

- [ ] GitHub에서 새 PR을 열면 템플릿이 자동으로 채워진다.
- [ ] 비개발자도 `docs/pr-guide.md`만 읽고 PR 규칙을 이해할 수 있다.
- [ ] 투자 추천 표현 금지 규칙이 체크리스트에 포함되어 PR마다 확인된다.
...
4. shared schema 정의
5. mock collector fixture 작성
6. mock analyzer fixture 작성
7. mock `/agents/analyze`
8. web 카드에 mock signal 표시

### Sprint 1 — E2E 데모 루프

1. DB compose + seed stocks
2. watchlist CRUD
3. signal run endpoint
4. `raw_evidence` 저장
5. `source_results` 저장
6. worker orchestration mock
7. latest signal API
8. dashboard update
9. journal create/list

### Sprint 2 — 실제 데이터 하나씩 붙이기

1. DART Collector 실제 호출 검증
2. DART Analyzer fixture 검증
3. Report Collector local PDF ingestion
4. Report Analyzer RAG 검증
5. Alternative Collector source 1개 검증
6. Alternative Analyzer 변화율 검증
7. partial/fallback 처리
8. source detail 화면

### Sprint 3 — 발표 가능한 완성도

1. 삼성전자 / SK하이닉스 / 네이버 fixture 정리
...
2. `services/agent-worker`에 source별 mock collector 추가
3. `services/agent-worker`에 source별 mock analyzer 추가
4. `services/agent-worker`에 Debate Aggregation과 `/agents/analyze` 추가
5. `services/main-server`에 `/api/signals/run/{stockCode}` 프록시/저장 mock 추가
