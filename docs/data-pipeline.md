...
- **구현됨**: Report 큐 경로 `collect_report → process_report → normalize_report → analyze_report → ml_infer` (결정론 밸류에이션 fact 추출). Report 분석 결과 ID는 `aggregate_ctx.source_analysis_result_ids`로 ML/Aggregator queue 입력까지 전달됩니다. 단, Aggregator가 `REPORT`를 최종 `score_breakdown` 소스로 수용하는 작업은 아직 남아 있습니다.
