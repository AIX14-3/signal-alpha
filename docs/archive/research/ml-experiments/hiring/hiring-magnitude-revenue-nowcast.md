...
> 
> **한 줄 결론: 매그니튜드는 방향과 마찬가지로 무신호(BH-FDR 생존 0). 그러나 매출 나우캐스팅은
> 단독 채용 실험 사상 첫 *확정* 신호 — 유니버스를 57→94종목으로 확대하자 BH-FDR 생존 2셀
> (volume+duty decision_tree rankIC +0.128 q=0.000·volume knn +0.110 q=0.050). 전 비선형/트리
> 모델 양(+)·전 선형 모델 음(−)의 일관된 구조, duty(직무 mix) 피처가 매출에선 기여. 효과는
> 모달(rankIC ~0.11)하나 다중검정을 넘은 진짜 리드 — 방향·매그니튜드의 완전 무신호와 질적 차이.**
...
## 3-bis. 매출 나우캐스팅 — 유니버스 확대(57→94) 재검정 ★확정★

57종목 결과가 "시사적이나 미확정"이라 횡단면 통계력을 키웠다: 채용 임계 완화(≥30공고·3년 →

**≥15공고·2년**)로 유니버스 57→**94종목**(매출 보유 82). 표본 620→**734**, 분기 28 동일.

무거운 메타모델(stacking/voting)·O(n³)는 제외, fast-6 + 트리앙상블(RF/GBM/HGB/extra_trees)

permutation(200). feature-set volume·volume+duty.

**결과(N=20 = 2 feature-set × 10모델): BH-FDR 생존 2 ★**

| feat | model | rankIC_xs | raw p | BH_q | 생존 |
| --- | --- | --- | --- | --- | --- |
| volume+duty | decision_tree | +0.128 | 0.000 | **0.000** | ✅ |
| volume | **knn** | +0.110 | 0.005 | **0.050** | ✅ |
| volume | decision_tree | +0.085 | 0.025 | 0.100 |  |
| volume | random_forest | +0.097 | 0.025 | 0.100 |  |
| volume+duty | knn | +0.112 | 0.025 | 0.100 |  |
| volume+duty | random_forest | +0.081 | 0.045 | 0.143 |  |
| (선형 lda/logistic/ridge) |  | 음(−) | >0.5 |  |  |

- **다중검정을 넘은 첫 신호.** 57→94 확대로 knn raw p 0.015→0.005, BH 미생존→생존 전환.
- 전 비선형/트리·거리 모델(decision_tree·knn·RF·extra_trees·HGB·grad_boost) **양(+)·p≤0.13**,
전 선형 모델 **음(−)** = 강한 비선형 구조(단일 best-of-노이즈 아님).

- **duty(직무 mix)가 매출에선 기여**: 최강 셀이 volume+duty decision_tree. 방향 검정에서
미지지였던 사용자 duty 가설이 *매출 타깃*에선 지지됨.

- 보수적 해석: decision_tree는 고분산이나 knn(안정·단순)이 q=0.050로 보강, 가족 전체 일관성이
근거. 효과크기 모달(rankIC ~0.11)·28분기 소표본 — "확정 리드"이지 "완성 알파"는 아님.
